...
/*
 *
 * This endpoint returns the "profile" of the given backend user.
 *
 */
/*
 *
 ...
 *
 */
if ( empty( $input[ 'profileId' ] ) ) {
	$response[ 'statusCode' ] = 4001;
	$response[ 'message' ] = 'Please provide a profile Id.';
...
try {
	...
	$profile = CRM\getUserProfile( $input[ 'profileId' ] );

	// If no profile was found
	if ( empty( $profile ) ) {
		$response[ 'statusCode' ] = 4002;
		$response[ 'message' ] = 'This user does not exist or has no assigned profile.';
		http_response_code( 400 );
		die( json_encode( $response ) );
	}
