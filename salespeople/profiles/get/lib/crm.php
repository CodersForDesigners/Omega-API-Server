 ...
 *
 * Error codes:
 * 	TODO: Handle the situation where a user has no profile
 *
 */
function getUserProfile ( $id ) {

	$recordType = 'settings/profiles';
	$endpoint = DATA::$apiUrl . "${recordType}/${id}";

	$responseBody = getAPIResponse( $endpoint, 'GET' );

	if ( empty( $responseBody ) || empty( $responseBody[ 'profiles' ] ) )
		return null;

	$profile = array_filter( $responseBody[ 'profiles' ][ 0 ] );
